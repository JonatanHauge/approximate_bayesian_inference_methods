...
with col1:
    ...
    with col3:
        ...
        method5 = st.checkbox("subspace_and_full_LL", value=False)
        methods = []
        if method1:
            methods.append("BBVI")
        if method2:
        ...
        if method4:
            methods.append("NF")
        if method5:
            methods.append("subspace_and_full_LL")

        num_transformations = []
        if method4:
            st.markdown("*Number of layers in flow*")
            num_trans_10 = st.checkbox("10 transformations", value=True)
            num_trans_30 = st.checkbox("30 transformations", value=False)
            if num_trans_10:
    ...
    with col4:
        T_type_dict = {"BBVI": [], "Langevin": [], "NF": [], "subspace_and_full_LL": []}
        if method1 or method2 or method4 or method5:
            st.markdown("**Select T-type**)")
        if method1:
            T_types_BBVI_fixed = st.checkbox("fixed (BBVI)", value=True)
            if T_types_BBVI_fixed:
                T_type_dict["BBVI"].append("fixed")
            T_types_BBVI_vary = st.checkbox("vary (BBVI)", value=False)
            if T_types_BBVI_vary:
                T_type_dict["BBVI"].append("vary")
        if method2:
            T_types_Langevin_fixed = st.checkbox("fixed (Langevin)", value=True)
            if T_types_Langevin_fixed:
                T_type_dict["Langevin"].append("fixed")
            T_types_Langevin_vary = st.checkbox("vary (Langevin)", value=False)
            if T_types_Langevin_vary:
                T_type_dict["Langevin"].append("vary")
        if method4:
            T_types_NF_fixed = st.checkbox("fixed (NF)", value=True)
            if T_types_NF_fixed:
                T_type_dict["NF"].append("fixed")
            T_types_NF_vary = st.checkbox("vary (NF)", value=False)
            if T_types_NF_vary:
                T_type_dict["NF"].append("vary")
        if method5:
            T_types_subspace_and_full_LL_fixed = st.checkbox("fixed (subspace_and_full_LL)", value=True)
            if T_types_subspace_and_full_LL_fixed:
                T_type_dict["subspace_and_full_LL"].append("fixed")
            T_types_subspace_and_full_LL_vary = st.checkbox("vary (subspace_and_full_LL)", value=False)
            if T_types_subspace_and_full_LL_vary:
                T_type_dict["subspace_and_full_LL"].append("vary")
...
# Button to plot
    ...
    fig = plot_all(nested_result_dict, dataset=dataset, methods=methods, 
                    ...
                    plot_by=plot_by, filter_by=filter_dict, metric=[metric])
