                ...
                for K in filter_by["K"]:
                    for m in metric:
                        if T_type == "fixed":
                            ...
                            ax.plot(prior_sigmas, y, 'o-', label=f"BBVI, {T_type} T, low_rank={low_rank}, K={K}", c=method_color_dict["BBVI"](col_idx))
                            col_idx += 2
                        else:
                ...
                for K in filter_by["K"]:
                    for m in metric:
                        if T_type == "fixed":
                            ...
                            ax.plot(low_ranks, y, 'o-', label=f"BBVI, {T_type} T, sigma={sigma}, K={K}", c = method_color_dict["BBVI"](col_idx))
                            col_idx += 2
                        else:
                ...
                for low_rank in filter_by["low_rank"]:
                    for m in metric:
                        if T_type == "fixed":
                            ...
                            ax.plot(K_list, y, 'o-', label=f"BBVI, {KT_dict[dataset][f'K={K}']}, sigma={sigma}, low_rank={low_rank}", c = method_color_dict["BBVI"](col_idx))
                            col_idx += 2
            ...
            for K in filter_by["K"]:
                for m in metric:
                    if T_type == "fixed":
                        ...
                        ax.plot(prior_sigmas, y, 'o-', label=f"Langevin, {T_type} T, K={K}", c=method_color_dict["Langevin"](col_idx))
                        col_idx += 2
                    else:
                ...
                for K in filter_by["K"]:
                    for m in metric:
                        if T_type == "fixed":
                            ...
                            ax.axhline(y, color = method_color_dict["Langevin"](col_idx),  linestyle='--', label=f"Langevin, {T_type} T, sigma={sigma}, K={K}")
                            col_idx += 2
                        else:
            ...
            for sigma in filter_by["prior_var"]:
                for m in metric:
                    if T_type == "fixed":
                        ...
                        ax.plot(K_list, y, 'o-', label=f"Langevin, {KT_dict[dataset][f'K={K}']}, sigma={sigma}", c = method_color_dict["Langevin"](col_idx))
                        col_idx += 2
...
def plot_Flows(ax, result_dict, dataset="mnist", T_types = ["fixed"], num_transformations = [10],
               plot_by="prior_var", filter_by={"low_rank":[0], "K":[10], "prior_var": [0.01]}, 
               metric=["Accuracy"]):
                ...
                for num_trans in num_transformations:
                    for m in metric:
                        if T_type == "fixed":
                            ...
                            ax.plot(prior_sigmas, y, 'o-', label=f"Flows, {T_type} T, K={K}, num_trans={num_trans}", c=method_color_dict["Flows"](col_idx))
                            col_idx += 2
                        else:
                    ...
                    for num_trans in num_transformations:
                        for m in metric:
                            if T_type == "fixed":
                                ...
                                ax.axhline(y, color = method_color_dict["Flows"](col_idx),  linestyle='--', label=f"Flows, {T_type} T, sigma={sigma}, K={K}, num_trans={num_trans}")
                                col_idx += 2
                            else:
                ...
                for num_trans in num_transformations:
                    for m in metric:
                        if T_type == "fixed":
                            ...
                            ax.plot(K_list, y, 'o-', label=f"Flows, {KT_dict[dataset][f'K={K}']}, sigma={sigma}, num_trans={num_trans}", c = method_color_dict["Flows"](col_idx))
                            col_idx += 2
    ...
    for _, T_type in enumerate(T_types):
        if plot_by == "prior_var":
            for K in filter_by["K"]:
                for m in metric:
                    if T_type == "fixed":
                        y = [result_dict[f"method=subspace_and_full_LL"][f"dataset={dataset}"][f"K={K}"][f"T={1}"][f"low_rank=0"][f"sigma={sigma}"][m] for sigma in prior_sigmas]
                        ax.plot(prior_sigmas, y, 'o-', label=f"subspace_and_full_LL, {T_type} T, K={K}", c=method_color_dict["subspace_and_full_LL"](col_idx))
                        col_idx += 2
                    else:
                        y = [result_dict[f"method=subspace_and_full_LL"][f"dataset={dataset}"][f"K={K}"][f"{KT_dict[dataset][f'K={K}']}"][f"low_rank=0"][f"sigma={sigma}"][m] for sigma in prior_sigmas]
                        ax.plot(prior_sigmas, y, 'o-', label=f"subspace_and_full_LL, {KT_dict[dataset][f'K={K}']}, K={K}", c=method_color_dict["subspace_and_full_LL"](col_idx))
                        col_idx += 2
                ...
                for K in filter_by["K"]:
                    for m in metric:
                        if T_type == "fixed":
                            ...
                            ax.axhline(y, color = method_color_dict["subspace_and_full_LL"](col_idx),  linestyle='--', label=f"subspace_and_full_LL, {T_type} T, sigma={sigma}, K={K}")
                            col_idx += 2
                        else:
                            y = result_dict[f"method=subspace_and_full_LL"][f"dataset={dataset}"][f"K={K}"][f"{KT_dict[dataset][f'K={K}']}"][f"low_rank=0"][f"sigma={sigma}"][m]
                            ax.axhline(y, color = method_color_dict["subspace_and_full_LL"](col_idx),  linestyle='--', label=f"subspace_and_full_LL, {KT_dict[dataset][f'K={K}']}, sigma={sigma}, K={K}")
                            col_idx += 2
            ...
            for sigma in filter_by["prior_var"]:
                for m in metric:
                    if T_type == "fixed":
                        ...
                        ax.plot(K_list, y, 'o-', label=f"subspace_and_full_LL, {T_type} T, sigma={sigma}", c = method_color_dict["subspace_and_full_LL"](col_idx))
                        col_idx += 2
                    else:
                        y = [result_dict[f"method=subspace_and_full_LL"][f"dataset={dataset}"][f"K={K}"][f"{KT_dict[dataset][f'K={K}']}"][f"low_rank=0"][f"sigma={sigma}"][m] for K in K_list]
                        ax.plot(K_list, y, 'o-', label=f"subspace_and_full_LL, {KT_dict[dataset][f'K={K}']}, sigma={sigma}", c = method_color_dict["subspace_and_full_LL"](col_idx))
                        col_idx += 2
...
def plot_all(result_dict, dataset = "mnist", methods = ["BBVI"], 
               T_types = {"BBVI": ["fixed"], "Langevin": ["fixed"]},  num_transformations = [10], 
               plot_by="prior_var", 
    ...
    if "subspace_and_full_LL" in methods:
        plot_subspace_and_full_LL(ax, result_dict, dataset=dataset, T_types=T_types["subspace_and_full_LL"], plot_by=plot_by, filter_by=filter_by, metric=metric)
