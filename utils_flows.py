...
class BBVI_NormalizingFlows(object):
    def __init__(self, model, theta_map, P, log_lik, K, step_size=1e-2, max_itt=2000, 
                 batch_size=None, seed=0, verbose=False, T = 1000, prior_sigma = 1, SWA = True, device = None, num_transformations = 2):
        ...
        self.m = nn.Parameter(torch.zeros(K, device = self.device) )
    ...
    def flow(self, z):
        ...
        for i in range(self.num_transformations):
            elbo_contribution += torch.log(torch.abs(1 + self.u[i]@((1 - torch.tanh(self.w[i] @ z + self.b[i])**2) * self.w[i])))
            z = z + torch.tanh(self.w[i] @ z + self.b[i]) * self.u[i]
    ...
    def compute_ELBO(self, X, y):
        ...
        z_K, elbo_contribution = self.flow(z_samples)
        
        w_samples = z_K @ self.P.T + self.theta_map   # shape: (, D)
        expected_log_prior_term = torch.sum(self.log_prior(z_K))  # shape: scalar
              
        expected_log_lik_term = self.N/batch_size*self.log_lik(self.model, self.params, X, y, w_samples)  # shape: scalar
        
        # compute ELBO
        ELBO = 1/self.T * expected_log_lik_term + expected_log_prior_term + self.compute_entropy(torch.exp(self.v))+ elbo_contribution
    ...
    def fit(self, train_loader, seed=0):
        ...
        for itt in tqdm(range(self.max_itt), desc='Training Progress', leave=True):
            ...
            if self.verbose and (itt + 1) % 100 == 0:  # Update every 100 iterations
                tqdm.write(f'\tIteration: {itt + 1}, ELBO: {np.mean(self.ELBO_history[(itt+1)-100:itt]):.2f}, Log Likelihood: {np.mean(self.log_like_history[(itt+1)-100:itt]):.2f}, '
                       f'Prior: {np.mean(self.log_prior_history[(itt+1)-100:itt]):.2f}, Entropy: {np.mean(self.entropy_history[(itt+1)-100:itt]):.2f}')
