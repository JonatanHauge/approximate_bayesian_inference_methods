...
for dataset in datasets:
    ...
    for K, T in hyper_param_dict[dataset]['K_T_list']:
        ...
        result_table[f'K={K}, T={T}'] = [acc, entropy, lpd, ece, mce, ood]
        ...
        for start_plot in start_plots:
            ...
            if save_fig:
                if SWA:
                    plt.savefig(f'plots/{dataset}_SWA_plotfrom={start_plot}_K={K}_T={T}_batch_size={batch_size}_step_size={step_size}_acc={acc:.3}_Entropy={entropy:.3f}_ECE={ece:.3f}_LPD={lpd:.3f}_OOD={ood:.2f}_MCE={mce:.3f}_sigma_init=exp(-1).png')
                    result_table.to_csv(f"results/{dataset}_SWA_K={K}_T={T}_batch_size={batch_size}_step_size={step_size}_iter_{max_itt}.csv")
                else:
                    plt.savefig(f'plots/{dataset}_plotfrom={start_plot}_K={K}_T={T}_batch_size={batch_size}_step_size={step_size}_acc={acc:.3}_Entropy={entropy:.3f}_ECE={ece:.3f}_LPD={lpd:.3f}_OOD={ood:.2f}_MCE={mce:.3f}_sigma_init=exp(-1).png')
                    result_table.to_csv(f"results/{dataset}_No_SWA_K={K}_T={T}_batch_size={batch_size}_step_size={step_size}_iter_{max_itt}.csv")
            #plt.show()